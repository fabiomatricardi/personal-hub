...
import { getSettings, getTelegramFilesStatus, runTelegramFilesNow, getTelegramFilesHistory, getTelegramFilesList } from '../composables/useApi.js'
...
onMounted(async () => {
  ...
  try {
    const [s, h, c, f] = await Promise.all([
      getTelegramFilesStatus(),
      getTelegramFilesHistory(),
      getSettings(),
      getTelegramFilesList(),
    ])
    status.value = s
    history.value = h
    config.value = c
...
async function runNow() {
  ...
  result.value = null
  try {
    result.value = await runTelegramFilesNow()
    const s = await getTelegramFilesStatus()
    status.value = s
    const h = await getTelegramFilesHistory()
    history.value = h
    const f = await getTelegramFilesList()
    savedFiles.value = f
  } catch (e) {
  ...
  <div class="view-container">
    ...
    <div class="config-card">
      ...
      <div class="config-grid">
        ...
        <div class="config-item">
          <span class="config-label">Save Directory</span>
          <span class="config-value" :class="config.telegram_files_dir ? 'configured' : 'not-configured'">
            {{ config.telegram_files_dir || 'Not configured' }}
          </span>
        </div>
        <div class="config-item">
          <span class="config-label">Cron Schedule</span>
          <span class="config-value">{{ config.files_cron || 'Not set' }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="status-card">
      ...
      <div class="status-grid">
        <div class="stat-item">
          <div class="stat-number">{{ status.files_saved || 0 }}</div>
    ...
    <div v-if="result" class="result-card" :class="result.errors?.length ? 'error' : 'success'">
      ...
      <div class="result-grid">
        <div class="result-item">
          ...
          <span class="result-value">{{ result.files_found ?? 0 }}</span>
        </div>
        <div class="result-item">
          <span class="result-label">Files Saved</span>
          <span class="result-value">{{ result.files_saved ?? 0 }}</span>
        </div>
        <div class="result-item">
          <span class="result-label">Duplicates</span>
          <span class="result-value">{{ result.duplicates_skipped ?? 0 }}</span>
        </div>
      </div>
    ...
    <div class="files-card">
      <h3>Saved Files</h3>
      <div v-if="savedFiles.length === 0" class="empty-state">No files saved yet.</div>
      <table v-else class="files-table">
        ...
        <tbody>
          <tr v-for="(file, i) in savedFiles.slice(0, 50)" :key="i">
            <td>{{ file.file_name }}</td>
            <td>{{ formatTimestamp(file.saved_at) }}</td>
      ...
      <table v-else class="history-table">
        ...
        <tbody>
          <tr v-for="(entry, i) in history.slice().reverse().slice(0, 20)" :key="i">
            ...
            <td>{{ entry.files_found ?? 0 }}</td>
            <td>{{ entry.files_saved ?? 0 }}</td>
            <td>{{ entry.duplicates_skipped ?? 0 }}</td>
